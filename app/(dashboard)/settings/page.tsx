    ...
    return (
        <div className="max-w-4xl mx-auto space-y-12 pb-20">
            <header>
                ...
                <p className="text-muted">Manage your account, themes, and personalized workspaces.</p>
            </header>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {/* Sidebar Nav */}
                <div className="space-y-1">
                    {['Account', 'Appearance', 'AI Intelligence', 'Notifications', 'Workspaces'].map((tab, i) => (
                        <motion.button
                        ...
                        >
                            ...
                            {activeTab === 'AI Intelligence' && (
                                <section className="glass p-8 space-y-8">
                                    ...
                                    <div className="flex items-center justify-between p-6 bg-primary-500/5 border border-primary-500/20 rounded-2xl">
                                        <div className="space-y-1">
                                            <h4 className="font-bold text-sm text-primary">Personal AI Configuration</h4>
                                            <p className="text-[10px] text-muted uppercase tracking-tighter">Toggle to use your own API keys and specific models</p>
                                        </div>
                                        <button
                                            onClick={() => setAiSettings({ ...aiSettings, use_custom_settings: !aiSettings.use_custom_settings })}
                                            className={`w-14 h-7 rounded-full relative transition-all duration-300 ${aiSettings.use_custom_settings ? 'bg-primary-600' : 'bg-[var(--bg-glass)] border border-[var(--border-color)]'}`}
                                        >
                                        ...
                                        <div className="space-y-8">
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                <div className="space-y-2">
                                                    <label className="text-[10px] font-black uppercase tracking-widest text-muted">AI Provider</label>
                                                    <div className="relative">
                                                        <select
                                                            value={aiSettings.provider}
                                                            onChange={(e) => setAiSettings({ ...aiSettings, provider: e.target.value })}
                                                            className="w-full h-12 bg-[var(--bg-glass)] border border-[var(--border-color)] rounded-xl px-4 outline-none focus:border-primary-500 appearance-none cursor-pointer text-primary"
                                                        >
                                                            <option value="ollama" className="bg-primary text-secondary">Ollama (Local)</option>
                                                            <option value="openai" className="bg-primary text-secondary">OpenAI (Cloud)</option>
                                                            <option value="gemini" className="bg-primary text-secondary">Google Gemini (Cloud)</option>
                                                            <option value="claude" className="bg-primary text-secondary">Anthropic Claude (Cloud)</option>
                                                        </select>
                                            ...
                                            {aiSettings.provider !== 'ollama' && (
                                                <div className="space-y-2 animate-in fade-in slide-in-from-top-2">
                                                    <label className="text-[10px] font-black uppercase tracking-widest text-muted">API Token / Key</label>
                                                    <div className="relative">
                                                        <input
                                                            type="password"
                                                            value={aiSettings.api_key || ''}
                                                            onChange={(e) => setAiSettings({ ...aiSettings, api_key: e.target.value })}
                                                            className="w-full h-12 bg-[var(--bg-glass)] border border-[var(--border-color)] rounded-xl px-4 pr-12 outline-none focus:border-primary-500 font-mono text-sm text-primary"
                                    ...
                                    <div className="space-y-4">
                                        <div className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-white/5">
                                            <div>
                                                ...
                                                <p className="text-[10px] text-gray-400 uppercase">Weekly progress reports and tips</p>
                                            </div>
                            ...
                            {activeTab === 'Workspaces' && (
                                <section className="glass p-8 space-y-6">
                                    ...
                                    <div className="space-y-4">
                                        {workspaces.map(ws => (
                                            <div key={ws.id} className="flex items-center justify-between p-4 bg-[var(--bg-glass)] rounded-xl border border-[var(--border-color)]">
                                                <div>
                                                    <p className="font-bold text-sm tracking-tight text-primary">{ws.title}</p>
                                                    <p className="text-[10px] text-muted uppercase">{ws.difficulty} • {ws.category}</p>
                                                </div>
                                                <button
                                        ...
                                        {workspaces.length === 0 && <p className="text-sm text-muted italic text-center">No workspaces created yet.</p>}
                                    </div>
                                </section>
                            )}
                        </motion.div>
